...
const setupHerokuApp = async () => {
    ...
    const appBase = 'apps/salesforce-pdf-generator';
    sh.cd(appBase);

    log(`*** Creating Heroku app ${chalk.bold(sh.env.HEROKU_APP_NAME)}`);
    log(`heroku apps:create ${sh.env.HEROKU_APP_NAME} --json --buildpack https://github.com/lstoll/heroku-buildpack-monorepo.git`);
